...
import Logo from "../../../assets/images/ph.png";
...
export default function PHReportsScreen() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const response = await fetch("https://donn-hypsicephalic-sapientially.ngrok-free.dev/hydrosense/api_sensor.php");
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.header, { marginBottom: 10 }]}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={[styles.headerTitle, { color: "#fff" }]}>pH Reports</Text>
        </View>
      </View>
